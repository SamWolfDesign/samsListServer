...
router.put("/update/:mainId", validateSession, function (req, res) {
    const permission = ac.can(req.user.role).updateAny('thread') && ac.can(req.user.role).updateOwn('thread');
    ...
    if(permission.granted) {
        ...
        .then((thread) => {
            if(thread){
                res.status(200).json({thread, message: "Alright, NOW we're in business!"})
            } else {
...
router.delete("/delete/:id", validateSession, function (req, res) {
    const permission = ac.can(req.user.role).deleteAny('thread') && ac.can(req.user.role).deleteOwn('thread');
    if(permission.granted){
